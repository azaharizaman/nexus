...
1. [Introduction and Architectural Mandate](#introduction-and-architectural-mandate)
2. [Defining Architectural Boundaries](#defining-architectural-boundaries)
3. [Standardized Package Naming Conventions](#standardized-package-naming-conventions)
4. [The "Where Does This Go?" Decision Guide](#where-does-this-go-decision-guide)
5. [Technical Refactoring Directives](#technical-refactoring-directives)
6. [Architectural Decomposition: Atomic Package Brainstorm](#architectural-decomposition)
...
The Nexus ERP project is built on the philosophy of **Maximum Atomicity**. Our goal is to ensure that the core business logic is encapsulated in small, highly focused, and reusable Laravel packages that can theoretically function independently of the main Nexus ERP application.

Nexus ERP aims to cut the initial development timeline for any developer or organization that wants to build an ERP system but can't afford the typical 6-12 month development cycle.

> **The Nexus ERP Motto:** *"What if the future of Enterprise Software is not to be bought but to be built."*
...
**The Atomic Rule:** All business logic that governs a single, independent domain of the ERP (e.g., UOMs, Serial Numbers, Currencies) MUST reside in its own package.

The Nexus ERP Core is responsible only for:

- **Orchestration** — Coordinating interactions between packages
- **Configuration** — Setting up the system environment  
- **API Presentation** — Exposing unified APIs to clients

---

## Defining Architectural Boundaries

When developing a new feature or refactoring existing code, the fundamental question to ask is:

> **"Is this core domain logic, or is this integration/presentation?"**
...
#### Atomic Package Responsibilities
...
| **Internal API Routes** | Packages MUST define their own API routes, but these are for internal use only (e.g., local testing or package-internal calls). These are NOT public-facing ERP APIs. | `GET /api/uom-management/uoms` |
...
Packages **must** communicate via **Contracts (Interfaces)** and **Events**. If Package A needs to use Package B:

1. Package A defines a PHP Interface (Contract) for the service it needs
2. The Nexus ERP Core binds the concrete implementation from Package B to that interface
3. Package A depends on the Contract, not the concrete implementation

---

### B. Nexus ERP Core (The "Orchestrator")

The main Nexus ERP application is the **API Presentation Layer** and the **Service Orchestrator**. It is a true headless application that only communicates to the external world via **API**, **GraphQL**, and **WebSockets**.

It contains the bare minimum code necessary to make the system function as a unified ERP. The main users are system integrators, developers, and machine-to-machine systems—rarely end users. The "Head" (frontend) must be developed by other developers before it can be made public.

#### Nexus ERP Core Responsibilities
...
|---|---|---|---|---|
| **`-master`** | Master Data Definition | Defines, stores, and validates the static core reference data for a domain. | Low Volatility (Reference data), High Reusability | `nexus-item-master`, `nexus-employee-master` |
| **`-management`** | Transactional State & Rules | Manages state changes, applies complex business rules, and persists transactional data within a domain. | High Volatility (Transactional records), State Persistence | `nexus-uom-management`, `nexus-inventory-management`, `nexus-workflow-management` |
| **`-interface`** | External Abstraction | Provides a stable façade (Contract) for interacting with complex internal logic or external systems. Hides underlying implementation complexity. | Medium Volatility (Swappable implementation logic) | `nexus-ledger-interface`, `nexus-payment-interface` |
| **`-engine`** | Stateless Execution | Executes pure, computational logic, algorithms, or rule-sets based on inputs, without maintaining persistence for the execution itself. | Stateless, High Performance/Computation | `nexus-workflow-engine`, `nexus-reporting-engine` |
...
| **4** | Is this an Interface/Contract that defines how a specific service should be consumed? | Yes, this promotes decoupling. | **The Atomic Package that consumes the service**, OR a thin `nexus-contracts` package |
...
## Technical Refactoring Directives
...
**❌ Bad:**
```php
new \Nexus\UomManagement\Services\UomConverter();
```

**✅ Good:**
```php
app(\Nexus\Contracts\UomConverterInterface::class)->convert(...)
```
...
### A. Architectural & Decoupling Core (System Foundation)
...
### B. Fundamental Master Data & Management Core (ERP Constants)
...
### C. Core Financial & Operational Packages (Business Engine)
...
| **`nexus-item-master`** | Single source of truth for all Products, Services, Price Lists, and Catalogs. |
...
### D. Universal Supporting Packages (Cross-Cutting Concerns)
...
| **`nexus-model-serialization`** | Unique serial number generation, versioning, status tracking, and voiding. |
...
These packages manage fundamental business transactions and resources (inventory, assets, time) that are critical for nearly every industry, even if specific implementation details differ slightly in the Core Orchestrator.
...
These packages are necessary for the vertical functionality required by a specific industry.
...
#### 8. General Practitioner Medical
...
|---|---|---|
| **Architectural Drift** | Packages start referencing concrete classes, leading to a "distributed monolith" and loss of atomicity. | **Mandatory Code Reviews:** All pull requests involving cross-package communication must be blocked unless Contracts and Events are utilized. Automated static analysis tools will check for forbidden dependencies. |
